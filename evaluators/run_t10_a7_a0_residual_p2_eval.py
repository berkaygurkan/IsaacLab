#!/usr/bin/env python3
"""Isaac-side A7 A0-residual P2 evaluation scaffold.

This guarded evaluator composes a frozen healthy A0 PPO policy with an offline
A7 teacher-gap residual adaptor on deployment-safe observations only:

    a0_action = A0(student_obs_t)
    residual_action = A7_residual(student_obs_history, a0_action)
    final_action = a0_action + alpha * residual_action
...
state, use health tokens, use A2/A5 actions, or modify task/P2 code.
"""
...
DEFAULT_A7_CHECKPOINT = "papers/conference/results/t10_a7_a0_residual_distill_full_h16_seed0/a7_a0_residual.pt"
...
EXPECTED_RESIDUAL_INPUT_MODE = "history_plus_a0_action"
TARGET_VX = 1.0
VELOCITY_FIELDS = [
    "step",
    "mean_vel_x",
...
class A7ResidualEvalError(ValueError):
    """Raised for invalid A7 residual evaluation state."""


def build_parser(*, add_app_launcher_args: bool = False) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run guarded Isaac-side A7 A0-residual P2 evaluation.")
    ...
    parser.add_argument("--a7_checkpoint", default=DEFAULT_A7_CHECKPOINT)
...
def validate_args(args: argparse.Namespace) -> None:
    if args.num_envs <= 0:
        raise A7ResidualEvalError("--num_envs must be > 0.")
    if args.num_steps <= 0:
        raise A7ResidualEvalError("--num_steps must be > 0.")
    if args.seed < 0:
        raise A7ResidualEvalError("--seed must be non-negative.")
    if args.history_len < 1:
        raise A7ResidualEvalError("--history_len must be >= 1.")
    if not math.isfinite(float(args.alpha)):
        raise A7ResidualEvalError("--alpha must be finite.")
    if args.fault_onset_step_min < 0 or args.fault_onset_step_max < 0:
        raise A7ResidualEvalError("fault onset bounds must be non-negative.")
    if args.fault_onset_step_min > args.fault_onset_step_max:
        raise A7ResidualEvalError("--fault_onset_step_min must be <= --fault_onset_step_max.")
    if args.fault_onset_mode != DEFAULT_ONSET_MODE:
        raise A7ResidualEvalError("Only random_uniform P2 onset is supported for this scaffold.")
    if args.execute_eval:
        for label, checkpoint in (("A0", args.a0_checkpoint), ("A7", args.a7_checkpoint)):
            checkpoint_path = resolve_repo_path(checkpoint)
            if not checkpoint_path.is_file():
                raise A7ResidualEvalError(f"{label} checkpoint does not exist: {repo_relative(checkpoint_path)}")


def print_preview(args: argparse.Namespace) -> None:
    print("[T10-A7-A0-RESIDUAL-P2 EVAL PREVIEW]")
    ...
    print("  policy_kind: a7_a0_residual")
    print(f"  task: {TASK}")
    print(f"  a0_checkpoint: {args.a0_checkpoint}")
    print(f"  a7_checkpoint: {args.a7_checkpoint}")
...
def torch_load_checkpoint(path: Path, *, map_location: str) -> dict[str, Any]:
    ...
    if not isinstance(checkpoint, dict):
        raise A7ResidualEvalError(f"checkpoint must be a dict, got {type(checkpoint).__name__}.")
    return checkpoint
...
def assert_finite_tensor(name: str, tensor: Any) -> None:
    ...
    if not torch.isfinite(tensor).all():
        raise A7ResidualEvalError(f"{name} contains NaN or Inf.")
...
def assert_finite_value(name: str, value: Any) -> None:
    ...
    if isinstance(value, float):
        if not math.isfinite(value):
            raise A7ResidualEvalError(f"{name} is NaN or Inf: {value}")
        return
    ...
    if isinstance(value, dict):
        ...
        return
    raise A7ResidualEvalError(f"{name} has unsupported metric type {type(value).__name__}.")
...
def load_a7_residual_policy(*, checkpoint_path: Path, history_len: int, device: Any) -> tuple[Any, dict[str, Any]]:
    import torch

    add_trainers_to_path()
    from train_t10_a7_a0_residual_distill import build_residual_mlp

    checkpoint = torch_load_checkpoint(checkpoint_path, map_location="cpu")
    metrics = checkpoint.get("metrics")
    if not isinstance(metrics, dict):
        raise A7ResidualEvalError(f"A7 checkpoint missing metrics metadata: {repo_relative(checkpoint_path)}")
    guardrails = checkpoint.get("guardrails") if isinstance(checkpoint.get("guardrails"), dict) else {}
    if metrics.get("no_nan_inf") is False:
        raise A7ResidualEvalError("A7 checkpoint metrics.no_nan_inf is false.")
    checkpoint_history_len = checkpoint.get("history_len", metrics.get("history_len"))
    if checkpoint_history_len is None or int(checkpoint_history_len) != int(history_len):
        raise A7ResidualEvalError(f"A7 checkpoint history_len expected {history_len}, got {checkpoint_history_len}.")
    for source_name, source in (("checkpoint", checkpoint), ("metrics", metrics)):
        ...
        if input_dim is not None and int(input_dim) != EXPECTED_OBS_DIM:
            raise A7ResidualEvalError(f"A7 {source_name}.input_dim expected {EXPECTED_OBS_DIM}, got {input_dim}.")
        if action_dim is not None and int(action_dim) != EXPECTED_ACTION_DIM:
            raise A7ResidualEvalError(f"A7 {source_name}.action_dim expected {EXPECTED_ACTION_DIM}, got {action_dim}.")
        if residual_dim is not None and int(residual_dim) != EXPECTED_RESIDUAL_DIM:
            raise A7ResidualEvalError(
                f"A7 {source_name}.residual_dim expected {EXPECTED_RESIDUAL_DIM}, got {residual_dim}."
            )
    ...
    if residual_input_mode != EXPECTED_RESIDUAL_INPUT_MODE:
        raise A7ResidualEvalError(
            f"A7 residual_input_mode expected {EXPECTED_RESIDUAL_INPUT_MODE!r}, got {residual_input_mode!r}."
        )
    for key in ("teacher_obs_used_as_input", "true_fault_state_used", "health_token_used", "a2_action_used", "a5_action_used"):
        if bool(metrics.get(key, False)):
            raise A7ResidualEvalError(f"A7 checkpoint guardrail violated: metrics.{key}=true")
    if bool(metrics.get("residual_head_used")) is not True:
        raise A7ResidualEvalError("A7 checkpoint metrics.residual_head_used must be true.")
    if bool(metrics.get("a0_action_used")) is not True:
        raise A7ResidualEvalError("A7 checkpoint metrics.a0_action_used must be true.")
    required_guardrails = {
    ...
    for key, expected in required_guardrails.items():
        if bool(guardrails.get(key)) is not expected:
            raise A7ResidualEvalError(f"A7 checkpoint missing/false guardrail: {key}.")
    if bool(metrics.get("a7_target_matches_teacher_minus_a0")) is not True:
        raise A7ResidualEvalError("A7 checkpoint target consistency flag is not true.")
    if float(metrics.get("a7_target_max_abs_consistency_error", 1.0)) != 0.0:
        raise A7ResidualEvalError("A7 checkpoint target consistency error is not zero.")
    state_dict = checkpoint.get("model_state_dict")
    if not isinstance(state_dict, dict):
        raise A7ResidualEvalError("A7 checkpoint missing model_state_dict.")
    ...
    with torch.inference_mode():
        ...
        if dummy_residual.ndim != 2 or dummy_residual.shape[-1] != EXPECTED_RESIDUAL_DIM:
            raise A7ResidualEvalError(
                f"A7 residual dim expected {EXPECTED_RESIDUAL_DIM}, got {tuple(dummy_residual.shape)}."
            )
        assert_finite_tensor("a7_dummy_residual", dummy_residual)

    metadata = {
        "a7_checkpoint_model_class": checkpoint.get("model_class"),
        "a7_checkpoint_history_len": int(checkpoint_history_len),
        "a7_checkpoint_guardrails_ok": True,
        "a7_checkpoint_metrics": metrics,
        "a7_checkpoint_guardrails": guardrails,
        "residual_input_mode": residual_input_mode,
    }
    return model, metadata


def load_a0_policy(*, vec_env: Any, checkpoint_path: Path, agent_cfg: Any, agent_cfg_dict: dict[str, Any]) -> tuple[Any, Any]:
    from rsl_rl.runners import DistillationRunner, OnPolicyRunner

    print(f"[T10-A7-P2] A0 runner construction start", flush=True)
    if agent_cfg.class_name == "OnPolicyRunner":
        ...
        raise A7ResidualEvalError(f"A0: unsupported runner class {agent_cfg.class_name!r}.")
    print(f"[T10-A7-P2] A0 checkpoint load start {repo_relative(checkpoint_path)}", flush=True)
    runner.load(str(checkpoint_path))
    print("[T10-A7-P2] A0 checkpoint load done", flush=True)
...
def select_policy_device(args: argparse.Namespace, env_device: Any) -> Any:
    ...
    if str(requested).startswith("cuda") and not torch.cuda.is_available():
        print("[T10-A7-P2 WARNING] CUDA unavailable; using CPU for A7 residual inference.", flush=True)
        requested = "cpu"
...
def done_mask_tensor(dones: Any, *, num_envs: int, device: Any) -> Any:
    ...
    if int(tensor.shape[0]) != num_envs:
        raise A7ResidualEvalError(f"done tensor length mismatch: expected {num_envs}, got {int(tensor.shape[0])}.")
...
def write_readme(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        ...
        f"- A0 checkpoint: `{summary['a0_checkpoint']}`",
        f"- A7 checkpoint: `{summary['a7_checkpoint']}`",
        f"- task: `{summary['task']}`",
...
def execute_rollout(args: argparse.Namespace) -> int:
    ...
    a0_checkpoint_path = resolve_repo_path(args.a0_checkpoint)
    a7_checkpoint_path = resolve_repo_path(args.a7_checkpoint)
    ...
    try:
        print("[T10-A7-P2 WARNING] Candidate-level evaluation only; not paper-grade final.", flush=True)
        print("[T10-A7-P2] config load start", flush=True)
        ...
        print("[T10-A7-P2] config load done", flush=True)

        print("[T10-A7-P2] gym.make start", flush=True)
        env = gym.make(TASK, cfg=env_cfg)
        print("[T10-A7-P2] gym.make done", flush=True)
        if isinstance(env.unwrapped, DirectMARLEnv):
            env = multi_agent_to_single_agent(env)

        print("[T10-A7-P2] P2 wrapper attach start", flush=True)
        ...
        if p2_wrapper.mapping.semantics != REQUESTED_SEMANTICS:
            raise A7ResidualEvalError(f"P2 actual semantics must be {REQUESTED_SEMANTICS}.")
        if p2_wrapper.fallback_used:
            raise A7ResidualEvalError("P2 fallback was used during wrapper attachment.")
        print("[T10-A7-P2] P2 wrapper attach done", flush=True)
        ...
        if int(vec_env.num_actions) != EXPECTED_ACTION_DIM:
            raise A7ResidualEvalError(f"action dim expected {EXPECTED_ACTION_DIM}, got {int(vec_env.num_actions)}.")
        ...
        a0_runner, a0_policy = load_a0_policy(
            vec_env=vec_env,
            checkpoint_path=a0_checkpoint_path,
            agent_cfg=agent_cfg,
            agent_cfg_dict=agent_cfg_dict,
        )
        a7_model, a7_metadata = load_a7_residual_policy(
            checkpoint_path=a7_checkpoint_path,
            history_len=args.history_len,
            device=policy_device,
        )
        policy_metadata = {
            "a0_policy_loaded": True,
            "a0_runner_class": agent_cfg.class_name,
            **a7_metadata,
        }
        ...
        print("[T10-A7-P2] rollout start", flush=True)
        for step_index in range(args.num_steps):
            step_number = step_index + 1
            student_obs = safe_student_obs(obs)
            with torch.inference_mode():
                a0_action = a0_policy(obs).detach().float()
                assert_action_shape("a0_action", a0_action, num_envs=tracking_num_envs)
                assert_finite_tensor("a0_action", a0_action)
                model_input = history_buffer
                assert_finite_tensor("model_input", model_input)
                residual_action = a7_model(model_input, a0_action.to(policy_device)).detach().float()
                ...
                final_action = a0_action.to(policy_device) + float(args.alpha) * residual_action
            ...
            base_norm = a0_action.norm(dim=1)
            ...
            if p2_wrapper.fallback_used or fallback_value > 0.0:
                raise A7ResidualEvalError("P2 fallback was used during evaluation.")
            ...
            if bool(fault_mask.any().item()):
                ...
                if scalar(log_values.get("P2/simulation_override_applied")) <= 0.0:
                    raise A7ResidualEvalError("P2 simulation override was not applied after onset.")
            ...
            if velocity_source is None:
                ...
                print(f"[T10-A7-P2] velocity_source={velocity_source}", flush=True)
            ...
            if step_index < 3 or step_number % 50 == 0 or step_number == args.num_steps:
                print(
                    f"[T10-A7-P2] step={step_number} mean_vel_x={mean_vel_x:.4f} "
        ...
        if not p2_fault_became_active:
            raise A7ResidualEvalError("P2 fault never became active during evaluation.")
        if not simulation_override_applied:
            raise A7ResidualEvalError("P2 simulation override was never observed after onset.")
        if not no_nan_inf:
            raise A7ResidualEvalError("NaN or Inf was observed during evaluation.")
        ...
        summary = {
            "eval_scope": "t10_a7_a0_residual_p2_eval",
            "created_at": datetime.now().isoformat(timespec="seconds"),
            "policy_kind": "a7_a0_residual",
            "a0_checkpoint": repo_relative(a0_checkpoint_path),
            "a7_checkpoint": repo_relative(a7_checkpoint_path),
            "task": TASK,
            ...
            "residual_input_mode": EXPECTED_RESIDUAL_INPUT_MODE,
            "action_composition": "final_action = a0_action + alpha * residual_action",
        ...
        if summary["observation_dim"] != EXPECTED_OBS_DIM:
            raise A7ResidualEvalError("summary observation_dim mismatch.")
        if summary["action_dim"] != EXPECTED_ACTION_DIM:
            raise A7ResidualEvalError("summary action_dim mismatch.")
        if summary["fallback_used"]:
            raise A7ResidualEvalError("summary indicates fallback was used.")
        if not summary["simulation_override_applied"]:
            raise A7ResidualEvalError("summary indicates simulation override was not applied.")
        if not summary["p2_fault_became_active"]:
            raise A7ResidualEvalError("summary indicates P2 fault never became active.")
        if not summary["no_nan_inf"]:
            raise A7ResidualEvalError("summary indicates NaN/Inf was observed.")
        ...
        for required_path in (summary_json, velocity_csv, output_dir / "command.txt", readme_path):
            if not required_path.is_file():
                raise A7ResidualEvalError(f"required output was not written: {repo_relative(required_path)}")
        print(f"[T10-A7-P2] summary: {repo_relative(summary_json)}", flush=True)
        print(f"[T10-A7-P2] velocity_timeseries: {repo_relative(velocity_csv)}", flush=True)
        print(f"[T10-A7-P2] README: {repo_relative(readme_path)}", flush=True)
        del a0_runner
        return 0
    finally:
...
def main() -> int:
    ...
    except A7ResidualEvalError as exc:
        print(f"[T10-A7-P2 ERROR] {exc}", file=sys.stderr, flush=True)
        return 2
    except Exception:
        traceback.print_exc()
        return 1
